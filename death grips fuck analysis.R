...
songs_with_fuck <- tidy_lyrics %>%
  ...
  group_by(artist, album, title) %>%
  summarize(n_fuck = n()) %>%
  full_join(song_list, by = c("artist", "album", "title")) %>%
  replace(is.na(.), 0) %>%
  arrange(desc(n_fuck))
...
# need to fix
songs_without_fuck <- song_list %>%
  anti_join(songs_with_fuck, by = c("artist", "album", "title"))

# need to fix
songs_without_curses <- song_list %>%
  anti_join(songs_with_curses, by = c("artist", "album", "title"))

curses_rate <- tidy_lyrics %>%
  group_by(artist, album, title) %>%
  summarize(n_words = n()) %>%
  left_join(songs_with_curses, by = c("artist", "album", "title")) %>%
  replace(is.na(.), 0) %>%
...
fuck_rate <- tidy_lyrics %>%
  group_by(artist, album, title) %>%
  summarize(n_words = n()) %>%
  left_join(songs_with_fuck, by = c("artist", "album", "title")) %>%
  replace(is.na(.), 0) %>%
...
songs_with_curses %>% 
  ...
  ggplot(aes(x = reorder(title %>% str_trunc(20), n_curse), y = n_curse, fill = album)) +
  geom_col() +
...
songs_with_fuck %>%
  ...
  ggplot(aes(x = title %>% str_trunc(20) %>% reorder(n_fuck), y = n_fuck, fill = album)) +
  geom_col() +
...
curses_rate %>%
  ...
  ggplot(aes(x = title %>% str_trunc(20) %>% reorder(curses_per_hundred_words), y = curses_per_hundred_words, fill = album)) +
  geom_col() +
...
fuck_rate %>%
  ...
  ggplot(aes(x = title %>% str_trunc(20) %>% reorder(fucks_per_hundred_words), y = fucks_per_hundred_words, fill = album)) +
  geom_col() +
...
songs_with_fuck %>%
  ungroup() %>%
  ggplot(aes(x = reorder(title %>% str_trunc(20), n_fuck), y = n_fuck, fill = album)) +
  geom_col(width = 0.9, show.legend = FALSE) +
  facet_wrap(~album, ncol = 3, scales = "free") +
...
fuck_rate %>%
  ungroup() %>%
  ggplot(aes(x = reorder(title %>% str_trunc(20), fucks_per_hundred_words), y = fucks_per_hundred_words, fill = album)) +
  geom_col(width = 0.9, show.legend = FALSE) +
  facet_wrap(~album, ncol = 3, scales = "free") +
...
curses_rate %>%
  ungroup() %>%
  mutate(title = str_trunc(title, 20)) %>%
  mutate(title = reorder(title, curses_per_hundred_words)) %>%
  ggplot(aes(x = title, y = curses_per_hundred_words, fill = album)) +
  geom_col(width = 0.9, show.legend = FALSE) +
  facet_wrap(~album, ncol = 3, scales = "free") +
